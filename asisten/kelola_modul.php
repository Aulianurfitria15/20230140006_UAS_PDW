...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['tambah_modul'])) {
    ...
    if (isset($_FILES['file_materi']) && $_FILES['file_materi']['error'] == 0) {
        ...
        if (move_uploaded_file($file_materi['tmp_name'], $upload_dir . $file_materi_name)) {
            ...
            $file_materi_name = ''; // Kosongkan nama file jika gagal upload
        }
    }
...
<div class="bg-white p-6 rounded-lg shadow-md mb-8">
    ...
    <form action="kelola_modul.php?id_praktikum=<?php echo $id_praktikum; ?>" method="POST" enctype="multipart/form-data">
        <div class="mb-4">
            ...
            <input type="text" id="nama_modul" name="nama_modul" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
        </div>
        <div class="mb-4">
            <label for="file_materi" class="block text-sm font-medium text-gray-700">File Materi (PDF, DOCX, PPTX)</label>
            <input type="file" id="file_materi" name="file_materi" class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100">
        </div>
        <button type="submit" name="tambah_modul" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
            Simpan Modul
        </button>
    </form>
</div>
                ...
                <?php if ($result_modul && $result_modul->num_rows > 0): ?>
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                            <!-- Tombol Edit bisa ditambahkan di sini dengan logika yang mirip kelola_matkul.php -->
