...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['id_update'])) {
        ...
        if (isset($_FILES['file_materi']) && $_FILES['file_materi']['error'] == 0) {
            ...
            $file_mater_name = time() . '_' . basename($file_materi['name']);
...
// 4. Logika untuk DELETE (Menghapus)
...
// 5. Logika untuk masuk mode EDIT
...
// 6. Logika untuk MEMBACA SEMUA DATA MODUL
...
<div class="bg-white p-6 rounded-lg shadow-md mb-8">
    <h3 class="text-xl font-bold text-gray-900 mb-4"><?php echo $is_edit_mode ? 'Edit Modul' : 'Tambah Modul Baru'; ?></h3>
    <form action="kelola_modul.php?id_praktikum=<?php echo $id_praktikum; ?>" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <input type="text" id="nama_modul" name="nama_modul" value="<?php echo htmlspecialchars($edit_data['nama_modul']); ?>" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md" required>
        </div>
        <div class="mb-4">
            <label for="file_materi" class="block text-sm font-medium text-gray-700">File Materi (PDF, DOCX, dll)</label>
            <?php if ($is_edit_mode && !empty($edit_data['file_materi'])): ?>
                ...
                <label class="block text-sm font-medium text-gray-700">Upload file baru untuk mengganti (opsional):</label>
            <?php endif; ?>
        ...
        <button type="submit" class="inline-flex justify-center py-2 px-4 border shadow-sm text-sm font-medium rounded-md text-white <?php echo $is_edit_mode ? 'bg-blue-600 hover:bg-blue-700' : 'bg-indigo-600 hover:bg-indigo-700'; ?>">
            <?php echo $is_edit_mode ? 'Update Modul' : 'Simpan Modul'; ?>
        </button>
        <?php if ($is_edit_mode): ?>
            <a href="kelola_modul.php?id_praktikum=<?php echo $id_praktikum; ?>" class="ml-3">Batal</a>
        <?php endif; ?>
    </form>
</div>
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4">
                            <?php if (!empty($row['file_materi'])): ?>
                                <a href="<?php echo $upload_dir . htmlspecialchars($row['file_materi']); ?>" target="_blank" class="text-indigo-600">Lihat File</a>
                            <?php else: ?>
                                -
                            <?php endif; ?>
                        </td>
                        <td class="px-6 py-4 text-center text-sm font-medium">
                            ...
                            <a href="kelola_modul.php?id_praktikum=<?php echo $id_praktikum; ?>&hapus_id=<?php echo $row['id']; ?>" class="text-red-600 hover:text-red-900" onclick="return confirm('Yakin ingin menghapus modul ini?');">Hapus</a>
                        </td>
                    </tr>
                    <?php endwhile; ?>
                <?php else: ?>
                    <tr>
                        <td colspan="3" class="px-6 py-4 text-center text-gray-500">Belum ada modul.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
